...
### v1.6 · 2025-11-28
#### ✨ 新增功能
- **智能 VPN 防护**：新增 IP 与时区比对机制，当检测到 VPN 环境时，优先尝试获取 GPS 真实位置，若失败则回退到默认城市（杭州），彻底解决“天气显示洛杉矶”的问题。
- **PWA 支持**：添加 `manifest.json` 和 `sw.js`，支持将网站安装到手机主屏幕（需 HTTPS 环境）。
- **音乐播放器升级**：默认最小化，支持移动端自动收起，修复了加载错误，并更新了播放列表。

#### 🚀 重构
- **模块化架构**：将 `main.js` 拆分为 `services/`（服务）、`components/`（组件）、`config.js`（配置）等独立模块，提升代码可维护性。
- **本地化支持**：全代码注释中文化，方便后续维护。
- **协议兼容**：优化模块加载方式，支持 `file://` 协议直接运行，无需本地服务器。

#### ✅ 修复
- **CORS 问题**：修复了本地打开时的跨域错误。
- **控制台清理**：移除了不必要的报错信息，优化了错误处理机制。
